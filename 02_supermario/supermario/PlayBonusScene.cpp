...
void PlayBonusScene::Update()
{
	if (m_pPlayer->m_currentScene != SCENE_PLAYBONUS) {
		m_pPlayer->SceneInit(SCENE_PLAYBONUS, m_charPos.x, m_charPos.y, false, m_pPlayer->m_CharLevel, false);
	...
	if (!m_gamePause) {
		...
		if (!m_pPlayer->m_collPause) {
			if (m_pPlayer->m_CharLevel == -1) {
				...
				GameManager::GetGameManager()->GetSceneManager()->DestroyScene("PlayBonusSceneScene");
			}
...
void PlayBonusScene::Render(HDC memDC, HDC backMemDC)
{
...
HBITMAP PlayBonusScene::ImageLoad(LPCWSTR path, int width, int height)
{
	return (HBITMAP)LoadImage(NULL, path, IMAGE_BITMAP, width, height, LR_LOADFROMFILE);
}

void PlayBonusScene::LoadImg(LPCWSTR path, int width, int height)
{
}

void PlayBonusScene::InitMapData()
{
	for (int i = 0; i < m_MapSize.y; ++i) {
		for (int j = 0; j < m_MapSize.x; ++j) {
			m_Map[i][j] = 0;
...
bool PlayBonusScene::CheckCollision(Object * object)
{
	// 플레이어 충돌처리
	if (object->m_Gravity > 0 && object->m_ItemType != 2) {
		...
		case 7:
			...
			dynamic_cast<Player*>(object)->m_totalScore += 200;
			break;
		}
	}
...
int PlayBonusScene::CheckCollisionTop(Object * object)
{
	POINT tempPoint;
	RECT tempRect;
	RECT tempCollRect;
		...
		if (IntersectRect(&tempCollRect, &tempRect, &object->m_Rect)) {
			...
			case 2:
				...
				if (object->m_CharLevel == 0) {
					m_Map[tempPoint.y][tempPoint.x] = 2;
...
int PlayBonusScene::CheckCollisionBottom(Object * object)
{
	POINT tempPoint;
	RECT tempRect;
	RECT tempCollRect;
...
int PlayBonusScene::CheckCollisionLeft(Object * object)
{
	POINT tempPoint;
	RECT tempRect;
	RECT tempCollRect;
...
int PlayBonusScene::CheckCollisionRight(Object * object)
{
	POINT tempPoint;
	RECT tempRect;
	RECT tempCollRect;
				...
				if (object->m_ObjectType == 0) {
					if (m_MapCollMap[tempPoint.y][tempPoint.x] == 6) {
						if (dynamic_cast<Player*>(object)->m_Dir == Direction::RIGHT) {
							...
							GameManager::GetGameManager()->GetSceneManager()->DestroyScene("PlayBonusScene");
							//object->m_UseCollision = false;
							object->m_PosX = 6560;
							m_pPlayer->SceneInit(SCENE_PLAY, object->m_PosX, object->m_PosY, true, m_pPlayer->m_CharLevel, false);
							m_pPlayer->ScrollInit(object->m_PosX, object->m_PosY);
							m_pPlayer->SceneChange(object->m_CharLevel);
						}
					}
				}
				return 2;
...
void PlayBonusScene::CollisionTop(Object * object)
{
	object->m_Gravity = 0;
	object->m_DirTopBottom = 0;
}

void PlayBonusScene::CollisionBottom(Object * object)
{
	object->m_state = 0;
	object->m_Gravity = 0;
...
void PlayBonusScene::CollisionLeft(Object * object)
{
	switch (object->m_ObjectType)
	{
	case 0:
...
void PlayBonusScene::CollisionRight(Object * object)
{
	switch (object->m_ObjectType)
	{
	case 0:
...
void PlayBonusScene::NoCollisionTop(Object * object)
{

}

void PlayBonusScene::NoCollisionBottom(Object * object)
{
	if (!object->m_state) object->m_state = true;
}

void PlayBonusScene::NoCollisionLeft(Object * object)
{
	object->m_canLeft = true;
}

void PlayBonusScene::NoCollisionRight(Object * object)
{
	object->m_canRight = true;
}

bool PlayBonusScene::CheckCollisionObject(Object * object, list<Object*>* objectList)
{
	RECT tempRect;
...
bool PlayBonusScene::CheckCollisionObject(list<Object*>* objectList)
{
	RECT temp;
...
bool PlayBonusScene::CheckCollisionObject(list<Object*>* objectList, list<Object*>* objectList2)
{
	RECT tempRect;
	iter = objectList->begin();
